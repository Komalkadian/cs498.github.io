function main() {
    ...
    d3.csv("country_wise_latest.csv").then(function(data){
            ...
            .attr("x", width/1.5).attr("y", height + 170).text("Regions");

            svg.append("text").attr("text-anchor", "end")
            .attr("x", width/5).attr("y", height-320).text("Total Deaths").attr("text-anchor", "start");
  ...
  .call(makeAnnotations)
    ...
    // create a tooltip
      ...
      const mousemove = function(event,d) {
        Tooltip
          .html("Total Deaths: " + d.TotalDeath+" <br>Region: "+ d.Region)
